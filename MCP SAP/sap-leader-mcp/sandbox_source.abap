  ...
  IF P_INSLOT IS NOT INITIAL.
    ...
    IF SY-SUBRC = 0 AND ( L_ART = 'Z02' OR L_ART = 'Z03' ).
      ...
      IF LT_SIB[] IS NOT INITIAL.
        ...
        LOOP AT LT_LOT.
          CLEAR L_MK.
          DATA: L_VORGLFNR TYPE QAMV-VORGLFNR.
          SELECT SINGLE MERKNR VORGLFNR INTO (L_MK, L_VORGLFNR) FROM QAMV
            WHERE PRUEFLOS = LT_LOT-PRUEFLOS
              AND VERWMERKM = P_MIC.
          IF SY-SUBRC = 0 AND L_MK IS NOT INITIAL.
            CLEAR: L_MW, L_AZ.
            IF P_MIC CS 'MVTR' OR P_MIC CS 'OTR'.
              DATA: BEGIN OF LT_QASE2 OCCURS 0,
                      PROBENR LIKE QASE-PROBENR,
                      MESSWERT LIKE QASE-MESSWERT,
                    END OF LT_QASE2.
              CLEAR LT_QASE2. REFRESH LT_QASE2.
              SELECT PROBENR MESSWERT INTO TABLE LT_QASE2
                FROM QASE
                WHERE PRUEFLOS = LT_LOT-PRUEFLOS
                  AND VORGLFNR = L_VORGLFNR
                  AND MERKNR = L_MK
                  AND ATTRIBUT = ''.
              SORT LT_QASE2 BY PROBENR DESCENDING.
              READ TABLE LT_QASE2 INDEX 1.
              IF SY-SUBRC = 0.
                L_SUM = L_SUM + LT_QASE2-MESSWERT. P_CNT = P_CNT + 1.
              ENDIF.
            ELSE.
              SELECT SINGLE MITTELWERT ANZWERTG INTO (L_MW, L_AZ)
                FROM QAMR
                WHERE PRUEFLOS = LT_LOT-PRUEFLOS AND MERKNR = L_MK AND VORGLFNR = L_VORGLFNR.
